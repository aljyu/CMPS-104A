...
// Helper function for emit_while that emits a child or expression 
// unless it's an operand
...
void emit_global_vars(astree* tree) {
    for (size_t childIdx = 0; childIdx < tree->children.size(); 
         childIdx++) {
...
// MSI: Maybe use block_nr of yyprse_astree instead?
    ...
    for (size_t index = 0; index < string_const_queue.size(); index++) {
        ...
        fprintf(oil_file, "char* s%d = %s;\n", charIndex++, 
                string_const_queue[index]->lexinfo->c_str());
    }
}

// Helper function for emit_struct
void emit_field_name(astree* child, const string* childName) {
    fprintf(oil_file, "f_%s_%s", childName->c_str(), 
            child->children.back()->lexinfo->c_str());
}
...
void emit_struct(astree* tree) {
    ...
    for (size_t child = 0; child < tree->children.size(); child++) {
        ...
        if (currChild->token_code == TOK_STRUCT) {
            ...
            for (size_t childIdx = 1; 
                 childIdx < currChild->children.size(); childIdx++) {
                fprintf(oil_file, "%s", eight_spaces.c_str());
                emit_field_name(currChild->children[childIdx], 
                                currChild->children[0]->lexinfo);
                fprintf(oil_file, ";\n");
            }
            fprintf(oil_file, "};\n");
        }
